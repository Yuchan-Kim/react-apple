  ...
  const getCartList = () => {
    ...
    axios({
      ...
      if (response.data.result === "success") {
        const sortedCartList = response.data.apiData.sort((a, b) => {
          if (a.seriesName === "악세사리" && b.seriesName !== "악세사리") return 1;
          if (a.seriesName !== "악세사리" && b.seriesName === "악세사리") return -1;
          return 0;
        });
        setCartList(sortedCartList); // 서버에서 가져온 카트 리스트 저장 및 정렬
        setQuantities(sortedCartList.map(item => item.count)); // 서버에서 가져온 수량을 설정
        setAppleCareSelected(sortedCartList.map(() => false)); // 애플케어 초기화
      } else {
        ...
        <div className="jm-checkout-page">
          <section className="jm-cart-summary">
            ...
            <div className="jm-continue-button">
              <button className="jm-checkout-button" onClick={handleMoveToPayform}>결제</button>
            </div>

            {/* 카트 아이템을 seriesName 기준으로 정렬하여 출력 */}
            {cartList
              .sort((a, b) => {
                if (a.seriesName === "악세사리" && b.seriesName !== "악세사리") return 1;
                if (a.seriesName !== "악세사리" && b.seriesName === "악세사리") return -1;
                ...
                <div className="jm-cart-item">
                  <div className="jm-item-image-box">
                    <img
                      ...
                      alt={cartVo.productName !== 'N/A' ? cartVo.productName : '제품 이미지'}
                  ...
                  <div className="jm-item-info">
                    ...
                    {/* 애플케어 UI는 seriesName이 "악세사리"가 아닐 때만 표시 */}
                    {cartVo.seriesName !== "악세사리" && (
                      <div className="jm-apple-care-section">
                        <div className="jm-as-icondetails-detail">
                          <div className='jm-apple-care-box'>
                            <h3 className='jm-apple-care-title'>
                          ...
                          <div className="jm-cart-shipping-info">
                            <div className='jm-pickup'>
                              <div className="jm-rc-segmented-control-icon">
                                {/* Apple Store Pickup Icon SVG */}
                                <svg viewBox="0 0 35 35" className="as-svgicon as-svgicon-applestorepickup as-svgicon-base as-svgicon-applestorepickupbase" role="img" aria-hidden="true" width="20px" height="20px">
                            ...
                            <div className='jm-delivery'>
                              <div className="jm-rc-segmented-control-icon">
                                {/* Delivery Icon SVG */}
                                <svg className="as-svgicon-rtl-mirrored as-svgicon as-svgicon-boxtruck as-svgicon-reduced as-svgicon-boxtruckreduced" viewBox="0 0 25 25" role="img" aria-hidden="true" width="20px" height="20px">
            ...
            <div className="jm-price-summary">
              ...
              <div className="jm-continue-button1">
                <button className="jm-checkout-button1" onClick={handleMoveToPayform}>결제</button>
              </div>
            </div>
          </section>
        </div>
